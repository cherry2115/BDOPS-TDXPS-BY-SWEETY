...
{ 
	...
	displayname = userDTO.getEmpName();
...
//------- plugin for google button ------//
tinymce.create('tinymce.plugins.googlePlugin', { 
	createControl: function(n, cm) {    
	 switch (n) {   
	 case 'google':  
	 var c = cm.createMenuButton('QC', {
	 title : 'google tool',  
	 image : '<%=request.getContextPath() %>/authoringtool/gui/images/googleicon.png', 
	 icons : false  , 
	 onclick : function () {  
		 google_tool();
	 }	
...
tinymce.PluginManager.add('google', tinymce.plugins.googlePlugin); 
//-------End of plugin for google button ------//
...
function msg(msg){
	...
	message = msg;
	var content = "";
	content = content + message.request.query.toUpperCase();
	var arr = message.resultList.result
	content = "<ul class=\"publist\">";
	for(i=0;i<arr.length;i++) {

	content = content +"<li>";
	  content = content + "<h3>"+arr[i].title+"</h3>";
	        content = content +"<ul>";

	            content = content +"<li>";
	                content = content + "<h4>AUTHORS: "+arr[i].authorString+"</h4>";
	            content = content +"</li>";

	            content = content +"<li>";
	                content = content + "<h4>SOURCE: "+arr[i].source+"</h4>";
	            content = content +"</li>";

	            content = content +"<li>";
	                content = content + "<h4>PMID: "+arr[i].pmid+"</h4>";
	            content = content +"</li>";

	            content = content +"<li>";
	                content = content + "<h4>DOI: "+arr[i].doi+"</h4>";
	            content = content +"</li>";

	            content = content +"<li>";
	                content = content + "<h4>JOURNAL TITLE: "+arr[i].journalInfo.journal.title+"</h4>";
	               
	            content = content +"</li>";

	            content = content +"<li>";
	                content = content + "<h4>AFFILIATION: "+arr[i].affiliation+"</h4>";
...
function open_pubmed() {
	...
	$('#mask').show();
	        var url = "http://www.ebi.ac.uk/europepmc/webservices/rest/search/query="+searchText+"&format=json&resulttype=core&callback=msg";
	        $.ajax({
...
function load_chapter_in_tinymce(filepath,batch,orderId,flow,bookName,monthYear,clOrderId,pFlow,articleORIssue){
...
function loadcontentofbook(){
	if (tinyMCE.activeEditor == null){
		window.setTimeout('loadcontent()',1000);
			...
			if(stagename != "AUTHOR_REVISION" && (stagename != "MASTER_COPY_EDITOR" || stagename=="PROOF_COLLATION")){
				
			viewStyleSheetofParticularBookChapter(batchNum,ordId,flo,bookN,moYear,clOrdId,pFlo,arORIssue);
...
function manage_clientspecific_tasks(){
	...
	if(((<%=userDTO.getRoleid().equalsIgnoreCase("RM1010")%> || <%=userDTO.getRoleid().equalsIgnoreCase("RM1011")%> ))){

		manage_user('authoringtool/finaltable/reviewarticletable.jsp');
		
	}
	else if(((<%=userDTO.getRoleid().equalsIgnoreCase("RM1010")%> || <%=userDTO.getRoleid().equalsIgnoreCase("RM1011")%> ))){
		manage_fms_oup();
...
function showstyleguidetab(){
	...
	var pdfserverpath = "<%=getPath%>/temp/files/"+itemcode+"/fms_readonly/OrdersDtd/stylesheet/"+itemcode+".pdf?random="+new Date().getTime();
...
$(document).ready(function () {
	...
	<% if(customer_account.equalsIgnoreCase("elsevierbookseries")){%>
	$('.sHundred').html('<img src="<%=request.getContextPath() %>/authoringtool/gui/images/firstproof.jpg">');
...
$(function(){ 
	...
	$('.showDocs').click(function () {				
        $('.docsContainer').toggle('slide', {direction: 'top'}, 500);				
    });
...
var typeofpdfopened;
 function autoopen3DFile(markername,type){
		if(markername=="MNT_SINGAPORE_JOURNAL_JOCS_12099_110"){type="main";}
 ...
 function refreshpdfqc(){
	 ...
	 <% if(customer_account.equalsIgnoreCase("elsevierbookseries")){%>
		...
		var pdfserverpath="<%=getPath%>temp/"+ordId+"/"+bookN+"/"+arORIssue+"_"+clOrdId+"_"+bookN+".pdf?random="+new Date().getTime();
 ...
 function publishxmlFile(filepath,nameoffile)
 {

//  	alert(filepath+' , '+nameoffile);
 	$('#load_pdf').html("<form id=\"xmlform\" method=\"post\" action=\"<%=request.getContextPath()%>/authoringtool/processcontent/forcedownloadxml.jsp?filename="+nameoffile+"\"><table width=\"100%\"><tr><td><input type=\"text\" id=\"indexvalue\" style=\"width:100%\"/></td><td><input id=\"searcherforxml\" type=\"button\" class=\"pdf_view\"  onclick=\"searchforerrorinxml();\" value=\"search\"></td><td><input id=\"downloadxmlfile\" type=\"button\" class=\"pdf_view\"  onclick=\"forcedownloadxmlfile();\" value=\"Download XML\"></td></tr></table></br><textarea cols=\"115\" rows=\"40\" id=\"xmldisplay\" name=\"xmldisplay\" width=\"100%\"  ></textarea></form>");
 	$('.PDFLeftContainer').show();
 	$('.StyleGuide').hide();
 	$('.PDFEdit').show();
 	$('.MsgNoArticleSelected').hide();
...
function showindexpdfforbook(){
	 var pdfserverpath="<%=getPath%>temp/files/"+itemcode+"/Index.pdf?random="+new Date().getTime();
	var path="pdfmaster/web/pdfviewer.jsp?pdfpath="+pdfserverpath;
	 	var iframepath= "<%=request.getContextPath()%>/"+path+"?random="+new Date().getTime();
	 	$('#load_pdf').html('<iframe src='+iframepath+' width="100%" height="100%" name=\"cleanproofframe\" id=\"cleanproofframe\">');
...
function showtocpdfforbook(){
	 var pdfserverpath="<%=getPath%>temp/files/"+itemcode+"/CaseIndex.pdf?random="+new Date().getTime();
	var path="pdfmaster/web/pdfviewer.jsp?pdfpath="+pdfserverpath;
	 	var iframepath= "<%=request.getContextPath()%>/"+path+"?random="+new Date().getTime();
	 	$('#load_pdf').html('<iframe src='+iframepath+' width="100%" height="100%" name=\"cleanproofframe\" id=\"cleanproofframe\">');
...
function showProgressforIndexCreation(){
	 ...
	 $.ajax({
		  ...
		  statusCode: {
			    404: function() {
					 $('#mask').show();
					 error_notice_pdf("Index creation for book is under process. Please wait for a while..");
					 setTimeout(showProgressforIndexCreation,2000);		
			    },
	 	200: function(msg) {
			    	$('#mask').hide();

			    	changeStagesOfChapterForParticularBook();
			    	load_table_again();
			    	showindexpdfforbook();
			    	success_notice("Index Created successfully.");
						    }
			  }
		});
...
function changeStagesOfChapterForParticularBook(){
	$.ajax({
		  type: "POST",
		  url: "<%=request.getContextPath()%>/authoringtool/processcontent/changeStagesOfChapterForParticularBook.jsp",
...
function submitfinalforindexingforbooks(){

	$.ajax({
		  type: "POST",
		  url: "<%=request.getContextPath()%>/authoringtool/processcontent/createmarkerforbooksubmit.jsp",
		  data: "filename="+itemcode ,
		  success: function(msg){
		
		success_notice("Book submitted Successfully");
		
		 $('#indexpdf').show();
		 //$('#tocpdf').show();
		
		showProgressforIndexCreation(); 
		spellcheckevent == "false";
	  }
		});
...
function submitfinalforauthorforbooks(){


	$.ajax({
		  type: "POST",
		  url: "<%=request.getContextPath()%>/authoringtool/processcontent/setstageofchapter.jsp",
		  data: "filename="+itemcode+"&chapterid="+chapterid ,
		  success: function(msg){
			  load_table_again();
			  success_notice("Chapter submitted Successfully...");
		  }
		});
...
function showprojectanalysis(){
	...
	$('#tabs_tdpms_btn').show();
	var jsppath="<%=request.getContextPath()%>/tdpms/jsp/projectanalysis.jsp?filename="+itemcode
	$('#load_jsptdpms').load(jsppath);
...
function moveToNextPage(){
	...
	$('#tabs_tdpms_btn').show();
	var jsppath="<%=request.getContextPath()%>/tdpms/jsp/furtherprojectanalysis.jsp";
	$('#load_jsptdpms').load(jsppath);
...
function showschedule(){
	$.ajax({
		  type: "POST",
		  url: "<%=request.getContextPath()%>/tdpms/jsp/processcontent/processshowschedule.jsp",
		  data: "filename="+itemcode ,
		  success: function(msg){
		 $('#load_jsptdpms').html(msg);
		 $('#load_pdf').hide();
		 
		success_notice("schedule retrieved succesfully");
	  }
		});
...
function getGanttchartImage(){

	$.ajax({
		  type: "POST",
		  url: "<%=request.getContextPath()%>/tdpms/jsp/GanttChartPreparation.jsp",
		  data: "filename="+itemcode ,
		  success: function(msg){
		 $('#load_jsptdpms').html(msg);
		 $('#load_pdf').hide();
...
function showFaq(){
	...
	window.open("<%=request.getContextPath() %>/authoringtool/externalfiles/FAQEditor.htm",'popup',"height=500,width=500,status=yes,toolbar=no,menubar=no,location=no,scrollbars=1");
}
    ...
    <div class="UserDetail fr">
      <ul>
        <li>
         ...
         <a id="FAQ" onclick="showFaq();" href="#"> FAQ</a>
        
        <a href="<%=request.getContextPath() %>/signin/login/login.jsp?user=0&pass=1" id="logout_msg">Logout</a><i class="icon-off icon-1x"></i></li>
      </ul>
    </div>
...
<div class="clear"></div>
 ...
 <div class="ProgreesBarContent">
            ...
            <div class="ProgreesBar">
               ...
               <!--  <div  id="step4div" class="Step4 red"><span class="hint--top" data-hint="PAGINATION">4</span></div>
                ...
                <div  id="step6div" class="Step6 red"><span class="hint--top" data-hint="PROOF COLLATION">6</span></div>  -->
 ...
 <div class="Dashboard" id="content_div">
    ...
    <div class="TitleContainer">
      <div class="Title" id="dashboardtitle">Welcome <%=userDTO.getEmpName().toUpperCase()%><%if(userDTO.getEmpName().equalsIgnoreCase("johnrigg")){%>FREELANCER<%}else{%><%-- <%=userDTO.getRole() %> --%><%}%></div>
      <div class="Icon_main">
        <!--  <div class="full_screen"> <input name="displaytdpms" id="displaytdpms" type="button" class="pdf_view" style="display: none;" onclick="displaytdpms();" value="Project Management"></div> -->
      <div class="full_screen"> <input name="" type="button" id="submitfinalforauthorforbooks" class="pdf_view" onclick="submitfinalforauthorforbooks();" style="display: none;" value="Submit to author"></div>
      
      <div class="full_screen"> <input name="" type="button" id="submitfinalforindexingforbooks" class="pdf_view" onclick="submitfinalforindexingforbooks();" style="display: none;" value="Submit Book"></div>
      ...
      <div class="full_screen"> <input name="" type="button" id="checklistp100" class="submit" onclick="displayp100checklist();" style="display: none;" value="checklist "></div>
        <div class="full_screen"> <input name="" type="button" class="submit" onclick="displaycitrixlogin();" style="display: none;" value="citrix "></div>
 ...
 <div class="load_jsptdpms" id="load_jsptdpms" style="display: none;">
      ...
      <div class="PDFLeftContainer"  style="background:#ffffff;">
        ...
        <div id="styleguide">
          ...
          <div class="PDFEdit fr " id="load_pdf" style="overflow:auto;"> 
      ...
      <div class="docsContainer"  id="docsview" name="docsview">
        <ul>
        
        <!-- for Pandt -->
	         <li id="artlog" style="display: none;"><a href="#" onclick="view_docs('artlog');" >Art log</a></li>
	         <li id="castoff" style="display: none;"><a href="#" onclick="view_docs('castoff');" >Cast off</a></li>   
	         <li id="designsample" style="display: none;"><a href="#" onclick="view_docs('designsample');" >Design sample</a></li>
	         <li id="analysisreport" style="display: none;"><a href="#" onclick="view_docs('analysisreport');" >Analysis report</a></li>
	         
	         
	         <!-- for stm -->
	         
	         <li id="stmstylechecklist" style="display: none;"><a href="#" onclick="view_docs('stmchecklist');" >Style checklist</a></li>
	         <li id="stmcastoff" style="display: none;"><a href="#" onclick="view_docs('stmcastoff');" >Cast off</a></li>
	         <li id="stmanalysisreport" style="display: none;"><a href="#" onclick="view_docs('stmanalysisreport');" >Analysis report</a></li>
	         
        </ul>
      </div>
        
         <input name="docs" id="docs" type="button" class="pdf_view showDocs" style="display: none;" onclick="" value="Reference Docs">
        
         <input name="indexpdf" id="indexpdf" type="button" class="pdf_view" style="display: none;" onclick="showindexpdfforbook();" value="Index PDF">
          <input name="tocpdf" id="tocpdf" type="button" class="pdf_view" style="display: none;" onclick="showtocpdfforbook();" value="TOC PDF">
        
         <input name="handbookpdf" id="handbookpdf" type="button" class="pdf_view" style="display: none;" onclick="showhandbookpdfforbook();" value="Handbook">
          <input name="passportpdf" id="passportpdf" type="button" class="pdf_view" style="display: none;" onclick="showpassportpdfforbook()();" value="Passport">
      
        
       <input name="displaypdf" id="displaypdf" type="button" class="pdf_view" style="display: none;" onclick="refreshpdfonroles();" value="View PDF">
         <input name="refreshpdf" id="refreshpdf" type="button" class="pdf_view" style="display: none;" onclick="refreshpdfqc();" value="PDF">
        <input name="extranet" id="extranet" type="button" class="pdf_view" onclick="openextranet();" style="display: none;" value="Extranet">
        <input name="capguide" id="capguide" type="button" class="pdf_view" onclick="opencapguide();" style="display: none;" value="Capguide">
        <input name="pdfview" id="pdfview" type="button" class="pdf_view" onclick="generatemssview();" style="display: none;" value="DAT">
         <input name="diff_pdf" id="diff_pdf" type="button" class="pdf_view" style="display: none;" onclick="opencleanpdf();" value="Diff PDF">
         <input name="showxml" id="showxml" type="button" class="pdf_view" value="Source" onclick="showXML('source');">
         <input name="orderxml" id="orderxml" type="button" class="pdf_view" value="Order" onclick="showXML('order');">
         <input name="carxml" id="carxml" type="button" class="pdf_view" value="Car" onclick="showXML('car');">
...
<!--           <input name="stylebutton" id="stylebutton" type="button" class="styles showStyle" value="Styles"> -->
      ...
      <div class="tdpmsLoader" id="load_tdpms"></div>
      ...
      <div class="ActionContainer" >
        <ul>
         <li id="viewcomments" style="display: none;"><a href="#" onclick="view_comments();" id="viewcommentshref" >View Comments</a></li>
            	...
            	<li id="submitfinalarticle" style="display: none;" ><a href="#" onclick="validateFinalSubmit();" id="submitfinalarticlehref">Submit Final Chapter</a></li>
            	...
            	<li id="submitfinalarticlebyme" style="display: none;" ><a href="#" onclick="validateVToolLogBeforeSubmission_ME();" id="submitfinalarticlebymehref">Submit Final Chapter</a></li>
                ...
                <li id="submitfinalarticlesme" style="display: none;" ><a href="#" onclick="final_submit_article_me();" id="submitfinalarticlesmehref" >Conversion(M.E.)</a></li>
                ...
                <li id="submitfinalarticlesqc" style="display: none;" ><a href="#" onclick="final_submit_article_qc();" id="submitfinalarticlesqchref">Submit Chapter(Q.C.)</a></li>
